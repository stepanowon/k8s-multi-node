...
$ kubectl apply -f ~/conf/ip-addr-pool.yaml
```

#### LoadBalancer 테스트
```sh
$ kubectl apply -f ~/conf/deployment.yaml
$ kubectl apply -f ~/conf/svc-lb.yaml
...
$ kubectl delete -f ~/conf/svc-lb.yaml
```
---
...
$ kubectl apply -f ~/conf/nodeapp1.yaml
$ kubectl apply -f ~/conf/nodeapp2.yaml

# ingress 파일 편집 : host 필드의 값을 앞에서 hosts 파일에 등록한 hostname(demo.example.com) 으로 변경
$ cat ~/conf/nodeapp-ingress.yaml
...
# ingresss 적용하기
$ kubectl apply -f ~/conf/nodeapp-ingress.yaml
```
...
#### 리소스 삭제
```sh
kubectl delete -f ~/conf/nodeapp1.yaml
kubectl delete -f ~/conf/nodeapp2.yaml
kubectl delete -f ~/conf/nodeapp-ingress.yaml
